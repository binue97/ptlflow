...
class NeXt1DEncoder(nn.Module):
    ...
    def _make_stem(self, hidden_chs: int, stem_stride: int, norm_layer):
        # Three 3x3 convs preserve the 7x7 receptive field of the original
        # single 7x7 conv while being more parameter-efficient.
        # Strides are distributed as (2, 2, 1) to achieve an overall stride of 4.
        assert stem_stride == 4, (
            f"Three-conv stem assumes stem_stride=4, got {stem_stride}"
        )
        return nn.Sequential(
            nn.Conv2d(3, hidden_chs // 2, kernel_size=3, stride=2, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(hidden_chs // 2, hidden_chs, kernel_size=3, stride=2, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(hidden_chs, hidden_chs, kernel_size=3, stride=1, padding=1),
            norm_layer(hidden_chs),
        )
