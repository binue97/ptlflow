...
class RAPIDFlow(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.export_use_bilinear_upsample = False
    ...
    def coords_grid(self, batch, ht, wd, dtype, device):
        ...
        if self.simple_io:
            # Stack in (x, y) order directly to avoid [::-1] reverse slice
            coords = torch.stack([coords[1], coords[0]], dim=0).to(dtype=dtype)
        else:
    ...
    def resize_flow_to_image(self, flow, image_shape):
        return F.interpolate(
            flow,
            image_shape,
            mode="bilinear",
            align_corners=not self.export_use_bilinear_upsample,
        )

    def resize_tensor(self, tensor, image_shape):
        return F.interpolate(
            tensor,
            image_shape,
            mode="bilinear",
    ...
    def forward(self, inputs):
        if self.simple_io:
            # In simple_io mode, skip preprocess_images to avoid pad/crop Slice ops.
            # Input dimensions must be divisible by output_stride.
            if isinstance(inputs, (tuple, list)) and len(inputs) == 2:
                x1_raw, x2_raw = inputs
            else:
                images = inputs
                if images.ndim == 5:
                    x1_raw = images[:, 0]
                    x2_raw = images[:, 1]
                elif images.ndim == 4 and images.shape[1] == 6:
                    x1_raw = images[:, :3]
                    x2_raw = images[:, 3:]
                else:
                    raise ValueError(
                        "simple_io expects either a tuple of two images, a [B,2,3,H,W] tensor, or a [B,6,H,W] tensor."
                    )

            x1_raw = (x1_raw - 0.5) * 2.0
            x2_raw = (x2_raw - 0.5) * 2.0
            image_resizer = None
        else:
        ...
        if self.simple_io:
            # Encode separately to avoid batch-dim Slice (TIDL compat)
        ...
        use_bilinear_upsample = self.export_use_bilinear_upsample
        ...
        for l, (x1, x2, cnet) in enumerate(
            ...
            if self.simple_io:
                # Direct indexing to avoid Slice with dynamic offsets (TIDL compat)
                net_tmp = cnet[:, :self.dec_net_chs]
                inp = cnet[:, self.dec_net_chs:self.dec_net_chs + self.dec_inp_chs]
            else:
            ...
            for k in range(iters_per_level[l]):
                ...
                get_mask = (not use_bilinear_upsample) and (
                    self.training
                    or (
                        l == (output_level - start_level)
                        and k == (iters_per_level[l] - 1)
                    )
                )
                ...
                if self.training:
                    if use_bilinear_upsample and l == (output_level - start_level):
                        out_flow = self.resize_flow_to_image(
                            out_flow,
                            [x1_raw.shape[-2], x1_raw.shape[-1]],
                        )
                    elif mask is not None and l == (output_level - start_level):
                        if self.simple_io:
                    ...
                    if use_bilinear_upsample:
                        ...
                        if self.simple_io:
                            ...
                            # Use matmul instead of sum(mul, dim) to avoid ReduceSum (TIDL compat)
                            # mask: [N,1,9,ps,ps,H,W] -> [N,1,1,ps,ps,H,W,9] (move dim2 to end)
                            # up_flow: [N,2,9,1,1,H,W] -> [N,2,9,1,1,H,W,1] (add dim for matmul)
                            mask_t = mask.permute(0, 1, 3, 4, 5, 6, 2).unsqueeze(-2)
                            up_flow_t = up_flow.permute(0, 1, 3, 4, 5, 6, 2).unsqueeze(-1)
                            up_flow = torch.matmul(mask_t, up_flow_t).squeeze(-1).squeeze(-1)
                            # up_flow: [N,2,ps,ps,H,W]
                            up_flow = up_flow.permute(0, 1, 4, 2, 5, 3)
                            up_flow = up_flow.reshape(
                                N, 2, pred_stride * H, pred_stride * W
                            )
                ...
                if not self.simple_io:
                    # Skip postprocess in simple_io mode to avoid crop Slice (TIDL compat).
                    # Input dims must be aligned to output_stride so no padding/crop is needed.
...
@trainable
@ptlflow_trained
class rapidflow_v1_it1(RAPIDFlow_it1):
    pass
...
@trainable
@ptlflow_trained
class rapidflow_v1_it2(RAPIDFlow_it2):
    pass
...
@trainable
@ptlflow_trained
class rapidflow_v1_it3(RAPIDFlow_it3):
    pass
...
@trainable
@ptlflow_trained
class rapidflow_v1_it6(RAPIDFlow_it6):
    pass
